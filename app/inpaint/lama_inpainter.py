...
class Inpainter:
    ...
    def inpaint(self, image: np.ndarray, boxes: list[BubbleBox]) -> np.ndarray:
        ...
        for cluster in clusters:
            ...
            local_boxes = [
                BubbleBox(b.x1 - cx1, b.y1 - cy1, b.x2 - cx1, b.y2 - cy1, b.confidence)
                for b in cluster
            ]
    ...
    def inpaint_mask(self, image: np.ndarray, mask: np.ndarray) -> np.ndarray:
        """Inpaint the region defined by a raw binary mask (0/255, same size as image)."""
        ys, xs = np.where(mask > 127)
        if len(ys) == 0:
            return image.copy()

        # Dilate mask by a few pixels to catch anti-aliased / edge pixels
    ...
    def _paint_region(self, image: np.ndarray, local_mask: np.ndarray, crop_box: tuple) -> np.ndarray:
        ...
        fill_color, ring_std, ring_range = self._sample_ring_bg(crop, local_mask, ring_px=15)
        # Flat fill ONLY if the ring is 100% uniform (std < 10 AND max-min brightness diff < 30).
        # If the mask crosses a boundary (e.g. dark outfit + white background), ring_range > 30,
        # forcing LaMa AI inpainting to reconstruct both background textures seamlessly.
        if ring_std < 10.0 and ring_range < 30.0:
            return self._flat_fill(image, local_mask, crop_box, fill_color)

        return self._lama_fill(image, crop, local_mask, crop_box)

    @staticmethod
    def _sample_ring_bg(crop: np.ndarray, local_mask: np.ndarray, ring_px: int = 15):
        """Sample background pixels in a ring around the mask for precise local color/std/range."""
        ...
        if ring_mask.sum() > 30:
            pixels = crop[ring_mask]
            gray = cv2.cvtColor(crop, cv2.COLOR_BGR2GRAY)[ring_mask]
            std_val = float(gray.std())
            range_val = float(gray.max() - gray.min())
            return pixels.mean(axis=0), std_val, range_val

        bg_mask = local_mask <= 127
        if bg_mask.sum() > 0:
            pixels = crop[bg_mask]
            gray = cv2.cvtColor(crop, cv2.COLOR_BGR2GRAY)[bg_mask]
            std_val = float(gray.std())
            range_val = float(gray.max() - gray.min())
            return pixels.mean(axis=0), std_val, range_val
